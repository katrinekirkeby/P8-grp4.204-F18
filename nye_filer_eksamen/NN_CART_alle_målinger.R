...
ggplot()+geom_point(aes(Model,Mean, size=Parametre))+geom_errorbar(aes(x=Model, ymin=Mean-sd, ymax=Mean+sd),width=0.25) + ylab('Krydsvalideringsscore')+geom_hline(yintercept=Mean[which.min(Mean)]+sd[which.min(Mean)], linetype='dashed')+scale_size_continuous(name='Kompleksitet')
